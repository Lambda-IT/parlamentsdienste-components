...
export class Navbar implements ComponentInterface {
    ...
    public render() {
        return <Host role="navigation">{this.init ? (!this.isMobile ? this.renderItems() : this.renderItemsMobile()) : ''}</Host>;
    }
        ...
        return (
            <div class="pd-navbar-item-mobile">
                ...
                <div class="pd-navbar-right">
                    ...
                    <pd-icon class="pd-navbar-icon pd-navbar-menu" name="menu" size={3} onClick={() => this.pdMenu.emit()}></pd-icon>
                </div>
            </div>
        );
    }
