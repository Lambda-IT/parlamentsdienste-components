...
Enables pd-sort, pd-filter-input, pd-filter-change events, which can trigger the external sorting functions.
...
| `pd-filter-change` | Gets emitted when externalRowHandling is true and the filter changes             | `CustomEvent<{}>`            |
| `pd-filter-input`  | TODO: Gets emitted when externalRowHandling is true and the filter input changes | `CustomEvent<string>`        |
| `pd-selected`      | Triggers when one or all rows get selected                                       | `CustomEvent<SelectedEvent>` |
| `pd-sort`          | Gets emitted when externalRowHandling is true a column gets sorted               | `CustomEvent<{}>`            |
